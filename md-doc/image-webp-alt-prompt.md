...
- 원본 확장자를 본문에 남기지 않는다.
- MDX를 먼저 수정하지 말고, 실제 디스크의 원본 파일 존재 여부부터 확인한다.
- 사용자가 `/images/...` 경로만 줘도 대응하는 `public/images/...` 실파일을 직접 확인한다.
- `.webp` 게시본이 없고 원본이 `png`, `jpg`, `jpeg`라면 질문 없이 `.webp` 변환을 먼저 수행한다.
- `.webp` 파일이 실제로 생성된 것을 확인하기 전에는 MDX에 `.webp` 경로를 넣지 않는다.
...
- 현재 요청 대상의 원본이 확인되면, 별도 질문 없이 `.webp` 변환 후 원본을 삭제하는 것을 기본값으로 본다.
- 삭제 전에는 `MDX 참조 경로 == 실제 생성된 .webp 경로`가 일치하는지 확인한다.
- 삭제 후에는 같은 이름의 원본 확장자 파일이 남아 있지 않은지 다시 확인한다.
...
1. 대상 URL/slug 확인
2. 파일명과 `-kr` / `-en` 확인
3. `/images/...` 경로를 받았으면 `public/images/...` 실파일부터 확인
4. 원본 확장자 확인
5. `.webp` 생성
6. 실제 `.webp` 생성 여부 확인
7. 게시 경로 확정
8. MDX의 `![]()`를 `![alt](/images/...webp)`로 교체
9. alt 품질 점검
10. 현재 요청에 직접 사용한 원본 삭제
11. MDX 참조 경로와 실제 파일 존재 여부 최종 검증
12. 최종 보고

강제 원칙:
- 순서를 바꾸지 않는다.
- `MDX 반영 -> 나중에 파일 확인` 순서로 처리하지 않는다.
- 원본이 있고 `.webp` 게시본이 없으면, 질문보다 변환/삭제/검증을 우선한다.
...
- 원본 삭제 여부
- 어떤 alt를 반영했는지
- MDX가 참조한 모든 `.webp` 파일이 실제로 존재하는지
- 참조 확장자와 실제 파일 확장자가 일치하는지
